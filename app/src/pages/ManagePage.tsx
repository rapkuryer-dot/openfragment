...
import { Search, AlertCircle, Wallet, Lock, Copy } from 'lucide-react';
...
import {
  buildTonApiJettonMetadataJson,
  validateHttpsAsciiMetadataUri,
...
export function ManagePage({
  ...
  void DEFAULT_JETTON_MAINNET;
  void DEFAULT_JETTON_TESTNET;
  const [contractAddr, setContractAddrRaw] = useState(initialAddress || '');
...
function JettonInfoCard({
  ...
  const friendlyJetton =
    tryFriendlyJettonAddress(contractAddr, network) ?? contractAddr.trim();
      ...
      <CardContent className="space-y-0">
            ...
            {info.adminAddress ? (
              <span className="inline-flex items-center gap-1.5">
                ...
                {isAdmin && (
                  <Badge
                    variant="secondary"
                    className="text-[10px] bg-[var(--success)]/10 text-[var(--success)] border-0"
                  >
                    You
                  </Badge>
                )}
        ...
        <div className="flex justify-between items-center py-2">
          ...
          <span className="font-mono text-[13px] font-semibold text-right">
            <a
              href={`https://verifier.ton.org/${contractAddr.trim()}`}
              target="_blank"
        ...
        {info.metadata.metadataUri && (
          <div className="flex justify-between items-center py-2">
            ...
            <a
              href={info.metadata.metadataUri}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[13px] font-semibold text-[#0098EA] hover:underline max-w-[65%] truncate text-right"
              title={info.metadata.metadataUri}
            >
              {shortenAddress(info.metadata.metadataUri)}
            </a>
          </div>
        )}

        {info.metadata.twitter && (
          <div className="flex justify-between items-center py-2">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              X / Twitter
            </span>
            <a
              href={info.metadata.twitter}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[13px] font-semibold text-[#0098EA] hover:underline max-w-[65%] truncate text-right"
              title={info.metadata.twitter}
            >
              {shortenAddress(info.metadata.twitter)}
            </a>
          </div>
        )}
        {info.metadata.telegram && (
          <div className="flex justify-between items-center py-2">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              Telegram
            </span>
            <a
              href={info.metadata.telegram}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[13px] font-semibold text-[#0098EA] hover:underline max-w-[65%] truncate text-right"
              title={info.metadata.telegram}
            >
              {shortenAddress(info.metadata.telegram)}
            </a>
          </div>
        )}
        {info.metadata.website && (
          <div className="flex justify-between items-center py-2">
            ...
            <a
              href={info.metadata.website}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[13px] font-semibold text-[#0098EA] hover:underline max-w-[65%] truncate text-right"
              title={info.metadata.website}
            >
              {shortenAddress(info.metadata.website)}
            </a>
          </div>
        )}

        <div className="pt-4 mt-1 border-t border-border space-y-2.5">
          ...
          {!walletAddress?.trim() && (
            <p className="text-center text-[11px] text-muted-foreground">
              Connect your wallet to open DeDust portfolio.
            </p>
          )}
        </div>
...
function WalletRequired({ tonConnectUI }: { tonConnectUI: TonConnectUI }) {
  return (
    <EmptyState
      ...
      action={
        <Button
          className="rounded-full max-w-[220px] mx-auto"
...
function AdminTab({
  ...
  const [newWebsite, setNewWebsite] = useState(info.metadata.website || '');
  const [newTwitter, setNewTwitter] = useState(info.metadata.twitter || '');
  const [newTelegram, setNewTelegram] = useState(info.metadata.telegram || '');
  ...
  async function handleRevokeAdmin() {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Admin revoke transaction sent!' });
      setTimeout(onSuccess, 5000);
    } catch (err) {
      setStatus({
        type: 'error',
  ...
  async function handleUpdateContent(e: FormEvent) {
    e.preventDefault();
    const uriErr = validateHttpsAsciiMetadataUri(newMetadataUri);
    if (uriErr) {
      setStatus({ type: 'error', message: uriErr });
      return;
    }
    ...
    for (const [label, val] of [
      ['Image URL', newImage],
      ['Website', newWebsite],
      ['X (Twitter) URL', newTwitter],
      ['Telegram URL', newTelegram],
    ] as const) {
    ...
    try {
      const body = await buildChangeContentBody({
        ...
        twitter: newTwitter.trim() || undefined,
        telegram: newTelegram.trim() || undefined,
        metadataUri: newMetadataUri.trim() || undefined,
      });
      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 300,
        network: network === 'mainnet' ? '-239' : '-3',
      ...
      <form onSubmit={handleUpdateContent} className="space-y-4.5">
        ...
        <div className="rounded-xl border border-border p-3.5 space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm font-semibold">TonAPI / Tonscan</div>
            <Button
              type="button"
              variant="secondary"
              ...
              onClick={async () => {
                try {
                  const json = buildTonApiJettonMetadataJson({
                    name: newName.trim() || 'Token',
                    symbol: newSymbol.trim() || 'TKN',
                    decimals: newDecimals.trim() || '9',
                  ...
                  setStatus({
                    type: 'success',
                    message:
                      'JSON copied. Host it over HTTPS, then paste the raw URL below.',
                  });
                  setTimeout(() => setStatus(null), 5000);
                } catch {
                  setStatus({ type: 'error', message: 'Could not copy' });
                }
              }}
            >
              <Copy className="size-4 mr-2" />
              Copy JSON for TonAPI
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="url"
              placeholder="https://…/my-jetton.json"
      ...
      <div className="mt-4 space-y-2">
        ...
        >
          {loading ? (
            ...
            'Revoke Admin (Permanent)'
          )}
        </Button>
        <p className="text-xs text-muted-foreground">
          This permanently removes admin rights from the minter. Minting,
          metadata updates, and upgrades will no longer be possible.
        </p>
      </div>
